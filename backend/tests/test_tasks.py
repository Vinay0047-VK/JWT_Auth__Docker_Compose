...
class TestTaskFilter:
    def test_filter_by_status(self, auth_client):
        auth_client.post('/api/tasks/', {'title': 'Todo task',     'status': 'todo'},        format='json')
        auth_client.post('/api/tasks/', {'title': 'Done task',     'status': 'done'},        format='json')
        auth_client.post('/api/tasks/', {'title': 'Progress task', 'status': 'in_progress'}, format='json')
...
class TestTaskUpdate:
    def test_update_own_task(self, auth_client, task):
        res = auth_client.patch(f'/api/tasks/{task["id"]}/', {
            'status': 'done'
        }, format='json')
        assert res.status_code == 200
    ...
    def test_cannot_update_other_users_task(self, second_auth_client, task):
        res = second_auth_client.patch(f'/api/tasks/{task["id"]}/', {
            'status': 'done'
        }, format='json')
    ...
    # now try to update without auth
        res = api_client.patch(f'/api/tasks/{task_id}/', {
            'status': 'done'
        }, format='json')
        assert res.status_code == 401
